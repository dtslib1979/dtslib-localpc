...
$stopHookScript = Join-Path $repoRoot "hooks" "stop-session-log.sh"
$startHookScript = Join-Path $repoRoot "hooks" "start-session-recovery.sh"

# Git Bash 경로로 변환 (Windows → /d/... 형식)
$stopHookUnix = $stopHookScript -replace '\\','/' -replace '^([A-Z]):',{ '/' + $_.Groups[1].Value.ToLower() }
$startHookUnix = $startHookScript -replace '\\','/' -replace '^([A-Z]):',{ '/' + $_.Groups[1].Value.ToLower() }
...
if (-not (Test-Path $stopHookScript)) {
    Write-Host "ERROR: Stop hook not found: $stopHookScript" -ForegroundColor Red
    exit 1
}
if (-not (Test-Path $startHookScript)) {
    Write-Host "ERROR: Start hook not found: $startHookScript" -ForegroundColor Red
    exit 1
}
...
foreach ($repo in $productionRepos) {
    ...
    if ($Uninstall) {
        ...
        # hooks 설정 추가/갱신 (SessionStart + Stop)
        $settings["hooks"] = @{
            SessionStart = @(
                @{
...
if ($Uninstall) {
    ...
    Write-Host ""
    Write-Host "  Stop hook:  $stopHookScript" -ForegroundColor Gray
    Write-Host "  Start hook: $startHookScript" -ForegroundColor Gray
