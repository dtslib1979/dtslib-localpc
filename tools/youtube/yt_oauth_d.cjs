...
const TOKEN_PATH  = path.join(__dir, 'accounts', 'token_d.json');
...
const ACCOUNT_EMAIL = 'dimas@dtslib.com';
...
(async () => {
  ...
  const profDir = path.join(require('os').homedir(), '.dtslib-youtube-profiles', 'account_d');
  ...
  try {
    ...
    console.log('   → token_d.json은 저장됨 (refresh_token 있음)');
  }
