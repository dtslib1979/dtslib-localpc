 ...
 * Thomas.tj.Park@gmail.com (Account A)
 */
'use strict';
...
const TOKEN_PATH  = path.join(__dir, 'accounts', 'token_c.json');
...
const ACCOUNT_EMAIL = 'Thomas.tj.Park@gmail.com';
...
(async () => {
  ...
  const profDir = path.join(require('os').homedir(), '.dtslib-youtube-profiles', 'account_c');
  ...
  if (body2.includes('이메일') || body2.includes('Email') || body2.includes('email') || body2.includes('Sign in')) {
    ...
    if (body3.includes('비밀번호') || body3.includes('password') || body3.includes('Password')) {
      ...
      await page.fill('input[type="password"]', PASSWORD).catch(async () => {
        ...
        if (pw) await pw.fill(PASSWORD).catch(() => {});
      });
      await page.keyboard.press('Enter');
      await sleep(5000);
    }
  ...
  for (let i = 0; i < 30; i++) {
    ...
    // delegation 페이지: 계정/브랜드 계정 선택 → 개인 구글 계정 클릭
    if (currentUrlNow.includes('/signin/oauth/delegation')) {
      console.log('  📋 delegation 페이지 — 개인 계정 클릭 시도...');
      // 방법1: 이메일 텍스트 기반 클릭
      const emailClick = await page.evaluate((email) => {
        ...
        return false;
      }, ACCOUNT_EMAIL).catch(() => false);
      if (emailClick) {
        console.log(`  ✅ 이메일 클릭: ${emailClick}`);
        await sleep(3000);
        continue;
      }
      // 방법2: Playwright locator로 이메일 텍스트 요소 클릭
      try {
        await page.locator(`text=${ACCOUNT_EMAIL}`).first().click({ timeout: 3000 });
        console.log('  ✅ locator 클릭 성공');
        await sleep(3000);
        continue;
      } catch (e) { /* 실패 시 계속 */ }
      // 방법3: 첫 번째 li 클릭
      await page.evaluate(() => {
        ...
        if (allLi.length > 0) allLi[0].click();
      }).catch(() => {});
      await sleep(3000);
      continue;
    }
  ...
  try {
    ...
    console.log('   → token_c.json은 저장됨 (refresh_token 있음)');
  }
